    ...
    <main className="min-h-screen bg-zinc-50 text-zinc-900 font-[family-name:var(--font-inter)]">
      ...
      <header className="sticky top-0 z-40 bg-zinc-50/80 backdrop-blur border-b border-zinc-200">
        <nav className="max-w-7xl mx-auto px-5 h-16 flex items-center justify-between">
          ...
          <a href={company.contact.phoneTel} className="max-md:hidden text-sm px-4 py-2 rounded-full bg-indigo-600 text-white hover:bg-indigo-700 transition">{company.contact.phone}</a>
        <MobileNav
            tone="light"
      ...
      <section className="max-w-7xl mx-auto px-5 pt-10">
        <div className="grid grid-cols-6 auto-rows-[140px] gap-4">
          ...
          <div className={`${tile} ${white} col-span-6 row-span-3 lg:col-span-4 lg:row-span-4 p-8 md:p-10 relative flex flex-col gap-6 lg:gap-0 lg:justify-between`}>
            ...
            {/* Keep both pills inline at every viewport. On mobile the tile
                interior is ~270px, so shrink padding + text-sm to ~222px
                combined width and they fit comfortably side-by-side rather
                than orphaning "Sprawdź ofertę" onto its own row. */}
            <div className="flex flex-wrap items-center gap-2 sm:gap-3">
              <a href={company.contact.phoneTel} className="px-4 sm:px-6 py-2.5 sm:py-3 text-sm sm:text-base rounded-full bg-zinc-900 text-white hover:bg-indigo-600 transition-colors">Zadzwoń</a>
              <a href="#oferta" className="px-4 sm:px-6 py-2.5 sm:py-3 text-sm sm:text-base rounded-full border border-zinc-300 hover:bg-zinc-100 transition-colors">Sprawdź ofertę</a>
            </div>
          </div>
        ...
        <div className="grid lg:grid-cols-12 gap-6 lg:items-start">
          ...
          <div className="lg:col-span-8 space-y-3">
            {faq.map((f, i) => (
              <FaqItem
                key={i}
                className={`${tile} ${white} data-[state=open]:bg-zinc-50 transition-colors`}
                question={
                  <span className="flex items-start justify-between gap-6 p-5">
                    <span className="font-semibold">{f.q}</span>
                    <span className="mt-1 w-7 h-7 rounded-full bg-zinc-100 grid place-items-center text-indigo-600 transition leading-none group-data-[state=open]/faq:rotate-45">+</span>
                  </span>
                }
